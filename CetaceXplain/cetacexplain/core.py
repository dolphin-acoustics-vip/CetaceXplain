...
from .audio import generate_wavfile
...
class ImageClassifierModel(ClassifierModel):
    ...
    def explain(self, test_images , multiclass_test_labels = None, background_images = None, 
        background_image_limit: int = 20, test_image_limit: int = 4, audio_explain: bool = False):
        ...
        audio_explain : bool, optional
            If True, audio versions of highlighted SHAP images for the classes predicted by the 
            machine learning model are generated and saved in the local directory.
        ...
        def get_frequency_arrays(scaling_factor = 10):
            """ This helper function gets values which should be indicative of the frequency 
            patterns outlined by SHAP's pixel highlighting.

            scaling_factor: factor by which all frequencies will be multiplied to ensure frequencies 
            are in an audible range
            """

            # get SHAP value matrices for only images which were predicted by the machine learning model
            predicted_shap_values = []
            for image_index, label_index in enumerate(predicted_label_indices):
                predicted_shap_values.append(shap_values[image_index][label_index])
            
            # get frequency arrays
            frequency_arrays = []
            for shap_matrix in predicted_shap_values:
                
                # For each column, the SHAP values for each row index within that column are summed up.
                # The position of the row with the largest sum is then taken for each column.
                # Each column may be thought of as representing a point in time along the horizontal axis 
                # of the spectrogram while each row may be considered as representing a frequency value along 
                # the vertical axis of the specetrogram.
                frequency_arrays.append(scaling_factor*np.argmax(shap_matrix.sum(axis=2), axis=0))

            return frequency_arrays

        # plot the image explanations
        shap.image_plot(shap_values, test_images, labels = labels)

        # generate audio versions of highlighted SHAP images for the classes predicted by the 
        # machine learning model if audio_explain == True 
        if audio_explain:
            frequency_arrays = get_frequency_arrays()
            for index, frequency_array in enumerate(frequency_arrays):
                generate_wavfile(f"whistle {index}.wav", frequency_array)
